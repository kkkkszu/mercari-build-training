...
　ans:GETはサーバーからデータを取得するために使う。URLにパラメータをつけてリソースを要求する。
　　　POSTはサーバーへデータを送信するときに使う。リクエストにデータを含めて送信する。
* ブラウザで `http://127.0.0.1:9000/items` にアクセスしても `{"message": "item received: <name>"}` が返ってこないのはなぜでしょうか？
　ans:データをサーバーに送信するには、-d オプションを使ってリクエストボディにパラメータを含める必要がある。/itemsエンドポイントはPOSTリクエスト専用であり、そのために、{"message": "item received: <name>"}が返ってこない。
* アクセスしたときに返ってくる**HTTPステータスコード**はいくつですか？
　ans:405 Method Not Allowed
* それはどんな意味をもつステータスコードですか？
　ans:（リクエストメソッドをそのリソースで使用できない時に返される。例えば、POSTメソッドしか許可されていないエンドポイントに GET リクdGET リクエストを送った場合など。）
